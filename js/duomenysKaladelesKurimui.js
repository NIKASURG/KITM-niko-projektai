const rasomTeks = [
  "",
  "SVEIKI ATVYKE Į MANO SVETAINE APIE MANE",
  "Nepamiršk pasisveikint, čia visada draugiški žmonės :)",
  "Jauti? Tai draugystės ir geros nuotaikos dvelksmas!",
  "JŪS VISADA ČIA LAUKEMAS",
  "NEBENT NESI",
  "TADA NELAUKEM TAVES ):",
  "TIKIOSI GRYŠI ):",
  "undefined",
  "Pagalvojot tai klaida :P",
  "Mes nerenkame jūsų duomenų",
  "GAL...",
  "Ar aš sakiau tu nelaukemas?",
  "Juokavau, tu esi nuostabus",
  "KĄ Tu Ką vAkaRe",
  "Geros dienos",
  "Ar jau radai savo mėgstamą kampelį šioje svetainėje?",
  "Sako, geriausi nuotykiai prasideda nuo smalsumo!",
  "Tu esi nuostab(us) - i ",
  "Džiaukis savimi",
];
const projektai = [
  {
    saltinis: "./seniProjektai/farm-shooter-slifavimas",
    aprasas:
      "Tai yra lapiu genocido žaidimas kurį kurėme kartu su programavimo bureliu",
    paveiksliuka: "./seniProjektai/paveiksliukai/lapiuSaudykle.png",
  },
  {
    saltinis: "./seniProjektai/kids1-Žaidimas",
    aprasas: "Tai yra mano pirmasis žaidimas padarytas su JS",
    paveiksliuka: "./seniProjektai/paveiksliukai/firstGame.png",
  },
  {
    saltinis: "./seniProjektai/discordProfiliniuGalerije-main",
    aprasas: "Tai yra svetaine kueioje dokumentuojamos mano discord profilines",
    paveiksliuka: "./seniProjektai/paveiksliukai/galerija.png",
  },
  {
    saltinis: "./seniProjektai/umJOOO",
    aprasas: "Tai yra projektas kur turi vesti komandas ir judinti personaža",
    paveiksliuka: "./seniProjektai/paveiksliukai/uhhhh.png",
  },
  {
    saltinis: "./seniProjektai/kids1-mcmod",
    aprasas: "Tai yra vieta kurioje laikomi privalomi minecraft serverio modai",
    paveiksliuka: "./seniProjektai/paveiksliukai/minecraft.png",
  },
  {
    saltinis: "./seniProjektai/GivuliuUkisReklama-main",
    aprasas: "Tai yra reklama knygai Givulių Ūkis",
    paveiksliuka: "./seniProjektai/paveiksliukai/ukis.png",
  },
];
const pyDarbai = [
  {
    saltinis: "./python/antras.py",
    aprasas: "Python kodas",
  },
  {
    saltinis: "./python/astuntas.py",
    aprasas: "Python kodas",
  },
  {
    saltinis: "./python/desimta.py",
    aprasas: "Python kodas",
  },
  {
    saltinis: "./python/devinta.py",
    aprasas: "Python kodas",
  },
  {
    saltinis: "./python/dvyliktas.py",
    aprasas: "Python kodas",
  },
  {
    saltinis: "./python/forAntras.py",
    aprasas: "Python kodas su for",
  },
  {
    saltinis: "./python/forKetvirtas.py",
    aprasas: "Python kodas su for",
  },
  {
    saltinis: "./python/forPenktas.py",
    aprasas: "Python kodas su for",
  },
  {
    saltinis: "./python/forpirmas.py",
    aprasas: "Python kodas su for",
  },
  {
    saltinis: "./python/forTrecias.py",
    aprasas: "Python kodas su for",
  },
  {
    saltinis: "./python/ifastoniolika.py",
    aprasas: "Python kodas su IF",
  },
  {
    saltinis: "./python/ifastunta.py",
    aprasas: "Python kodas su IF",
  },
  {
    saltinis: "./python/ifdvidesimtpirma.py",
    aprasas: "Python kodas su IF",
  },
  {
    saltinis: "./python/ifpirma.py",
    aprasas: "Python kodas su IF",
  },
  {
    saltinis: "./python/iftrylika.py",
    aprasas: "Python kodas su IF",
  },
  {
    saltinis: "./python/index.py",
    aprasas: "Šeip python kodas",
  },
  {
    saltinis: "./python/JAVASCRIPT.py",
    aprasas: "Šeip python kodas",
  },
  {
    saltinis: "./python/jonukoProblema.py",
    aprasas: "Jonuko problema",
  },
  {
    saltinis: "./python/ketvirtas.py",
    aprasas: "Šeip python kodas",
  },
  {
    saltinis: "./python/master.py",
    aprasas: "Fobonačio skaičius",
  },
  {
    saltinis: "./python/index.py",
    aprasas: "Šeip python kodas",
  },
  {
    saltinis: "./python/minecraftNamas.py",
    aprasas: "Minecraft namas (Geresnis)",
  },
  {
    saltinis: "./python/minecraftNamasNeTas.py",
    aprasas: "Minecraft namas (Blogesnis)",
  },
  {
    saltinis: "./python/pagrindinis.py",
    aprasas: "Šeip kodas",
  },
  {
    saltinis: "./python/penktas.py",
    aprasas: "Akvariumo kaina",
  },
  {
    saltinis: "./python/pirmas.py",
    aprasas: "pirmasDarbas",
  },
  {
    saltinis: "./python/septintas.py",
    aprasas: "Septinta užduotis",
  },
  {
    saltinis: "./python/sestas.py",
    aprasas: "Šešta užduotis",
  },
  {
    saltinis: "./python/taip.py",
    aprasas: "Draugo ND",
  },
  {
    saltinis: "./python/trecias.py",
    aprasas: "Trecai užduotis",
  },
  {
    saltinis: "./python/vanuoliktas.py",
    aprasas: "Venuolikta užduotis",
  },
  {
    saltinis: "./python/whileAntras.py",
    aprasas: "while užduotis",
  },
  {
    saltinis: "./python/whileDesimtas.py",
    aprasas: "while užduotis",
  },
  {
    saltinis: "./python/whileDevinioliktas.py",
    aprasas: "while užduotis",
  },
  {
    saltinis: "./python/whileDvidesimtVienas.py",
    aprasas: "while užduotis",
  },
  {
    saltinis: "./python/whilePirmas.py",
    aprasas: "while užduotis",
  },
  {
    saltinis: "./python/whileTecias.py",
    aprasas: "while užduotis",
  },
  {
    saltinis: "./python/ZiemojantysPauksciai.py",
    aprasas: "Žiemojantys paukščiai",
  },
  {
    saltinis: "./python/Atsiskaitymas/pirmas.py",
    aprasas: "Atsiskaitymas užd Nr1",
  },
  {
    saltinis: "./python/Atsiskaitymas/antras.py",
    aprasas: "Atsiskaitymas užd Nr2",
  },

  {
    saltinis: "./python/Atsiskaitymas/trecias.py",
    aprasas: "Atsiskaitymas užd Nr3",
  },
];
const svetaines = [
  {
    saltinis: "./svetaine/nebereikalingos_svetaines/HTML/index.html",
    aprasas: "Pirmojo atsiskaitymo index svetainė",
  },
  {
    saltinis: "./svetaine/nebereikalingos_svetaines/HTML/nexus.html",
    aprasas: "Nexus, mes pasakome ateitį ",
  },
  {
    saltinis: "./svetaine/nebereikalingos_svetaines/HTML/darSugalvosiu.html",
    aprasas: "Nebuvo idėju tai padariau žaidima",
  },
  {
    saltinis:
      "./svetaine/nebereikalingos_svetaines/atmestos_svetaines/index.html",
    aprasas: "Pirmojo atsiskaitymo svetaine kuri man nepatiko",
  },
  {
    saltinis: "./svetaine/grid/index.html",
    aprasas: "Darbas su grid",
  },
  {
    saltinis: "./svetaine/grid/antras.html",
    aprasas: "cooler Darbas su grid",
  },
  {
    saltinis: "./svetaine/flexAtsiskaitimasSvetaine/index.html",
    aprasas: "Darbas per atostogas su flex",
  },
  {
    saltinis: "./svetaine/divFlex/index.html",
    aprasas: "Mokausi naudoti flex",
  },
  {
    saltinis: "./svetaine/divFlex/flex2.html",
    aprasas: "Mokausi naudoti flex 2",
  },
  {
    saltinis: "./svetaine/divFlex/flex3.html",
    aprasas: "Mokausi naudoti flex 3 BLOGIO IŠKILIMAS",
  },
];
